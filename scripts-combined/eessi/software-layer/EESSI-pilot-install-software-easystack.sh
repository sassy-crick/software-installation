...
if [[ -z ${EESSI_SOFTWARE_SUBDIR} ]]; then
    ...
    echo ${EESSI_SOFTWARE_SUBDIR} > /dev/shm/EESSI_SOFTWARE_SUBDIR
fi
...
echo "Right now there seems to be a problem though"
ok_msg="Hurray, EasyStack worked!"
fail_msg="Ooops, that did not work. More work needs to be done it seems. :-("
cat ${TOPDIR}/../software/easystack.yml
$EB -d --experimental --easystack ../software/easystack.yml
check_exit_code $? "${ok_msg}" "${fail_msg}"
